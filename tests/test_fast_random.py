...
def make_state_array(fg: FastGenerator, n_agents: int, base_seed: int = 0) -> np.ndarray:
...
class TestVectorRandomStandardUniform:
    ...
    def test_selected_positions_shape(self):
        ...
        result = fg.vector_random_standard_uniform(state, selected_positions=sel, shape=4)
    ...
    def test_selected_positions_values_in_unit_interval(self):
        ...
        result = fg.vector_random_standard_uniform(state, selected_positions=sel, shape=20)
        assert np.all(result >= 0.0)
        assert np.all(result < 1.0)
...
class TestVectorRandomStandardNormal:
    ...
    def test_selected_positions_shape(self):
        ...
        state = make_state_array(fg, n)
        sel = np.array([0, 2, 5], dtype=np.intp)
        result = fg.vector_random_standard_normal(state, selected_positions=sel, shape=4)
        assert result.shape == (3, 4)
